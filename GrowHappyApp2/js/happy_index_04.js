	...
	function nextPage(){
		...
		register();
			...
			function register() {
				...
				mui.ajax(ip + "newbegin/index/modHappyMS", {
					data: {
						indexPage: indexPage,
						type:type	
					},
...
function goNextPage(i){
	...
	if(1==type){
		plus.storage.setItem("type", "名师讲学");
	}
	if(2==type){
		plus.storage.setItem("type", "学习分享");
	}
	if(3==type){
		plus.storage.setItem("type", "微课堂");
	}
	//类型
	plus.storage.setItem("htype1", type.toString());
	showFact('happy_detail_ms.html', '首页', '', 'happy_detail_ms','');
}
